...
const ProfileArchetype = () => {
  ...
  const checkAuthAndLoadProfile = async () => {
    try {
      ...
      // Charger le profil pour récupérer le genre
      const { data: profile, error: profileError } = await supabase
        .from('profiles')
        .select('gender, personal_definition')
        .eq('user_id', user.id)
        .maybeSingle();
      ...
      if (profile?.personal_definition && Array.isArray(profile.personal_definition)) {
        setSelectedArchetypes(profile.personal_definition);
      }
  ...
  const handleSaveArchetypes = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from('profiles')
        .update({
          personal_definition: selectedArchetypes,
        })
        .eq('user_id', user.id);
      ...
      if (error) {
        ...
        toast({
          title: "Erreur",
          description: "Impossible de sauvegarder vos archétypes",
          variant: "destructive",
        });
        setIsLoading(false);
        return;
      ...
      toast({
        title: "Succès",
        description: "Archétypes sauvegardés avec succès",
      });

      // Navigation vers la prochaine page
      navigate('/profile-archetype-preferences');
...
export default ProfileArchetype;