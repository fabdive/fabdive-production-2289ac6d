...
const ProfileSkinColor = () => {
    ...
    try {
      ...
      if (preferences?.preferred_skin_colors && preferences.preferred_skin_colors.length > 0) {
        if (preferences.preferred_skin_colors.includes('prefer_not_to_answer')) {
          ...
          setSelectedSkinColor(preferences.preferred_skin_colors[0]);
        }
      }
  ...
  const selectSkinColor = (colorId: string) => {
  ...
  const handleSavePreferences = async () => {
    if (!preferNotToAnswer && !selectedSkinColor) {
      toast({
        title: "Erreur",
        description: "Veuillez sélectionner une couleur de peau ou cocher 'je préfère ne pas répondre'",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const preferencesToSave = preferNotToAnswer ? ['prefer_not_to_answer'] : [selectedSkinColor];
      ...
      navigate('/profile-skin-color-preferences');
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center justify-center min-h-full px-6 text-center pt-16">
          ...
          <div className="mb-8">
            ...
            >
              ...
              <span className="text-lg font-comfortaa" style={{ color: '#fffae6' }}>
                Je préfère ne pas répondre ou
              </span>
            </button>
          </div>

          {/* Skin color options */}
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 mb-12 animate-fade-in-2">
            <div className="grid grid-cols-2 gap-6 w-full max-w-sm mx-auto">
              {skinColorOptions.map((option) => (
                <button
                  key={option.id}
                  onClick={() => selectSkinColor(option.id)}
                  disabled={preferNotToAnswer}
                  className="flex flex-col items-center justify-center transition-all disabled:opacity-50"
                >
                  {/* Color circle */}
                  <div 
                    className={`mb-3 p-2 rounded-full transition-all ${
                      selectedSkinColor === option.id && !preferNotToAnswer
                        ? 'ring-4 ring-fabdive-button' 
                        : 'hover:bg-white/10'
                    }`}
                    style={selectedSkinColor === option.id && !preferNotToAnswer ? { backgroundColor: '#14018d' } : {}}
                  >
                    <img
                      src={option.image}
                      alt={option.label}
          ...
          <button
            ...
            disabled={isLoading || (!preferNotToAnswer && !selectedSkinColor)}
...
export default ProfileSkinColor;