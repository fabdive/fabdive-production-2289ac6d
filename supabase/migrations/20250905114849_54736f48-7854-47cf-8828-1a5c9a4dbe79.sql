-- Corriger l'alerte en supprimant d'abord la fonction existante

-- 1. Supprimer la politique qui expose trop de données
...
-- 3. Recréer la fonction avec moins de données sensibles exposées
...
BEGIN
  ...
  -- Masquer skin_color et autres données sensibles
  RETURN QUERY
  SELECT 
    p.user_id,
    p.display_name,
  ...
  LIMIT 50; -- Limiter le nombre de profils
END;
$$;