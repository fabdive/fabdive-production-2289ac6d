...
DROP POLICY IF EXISTS "Users can view limited crush data" ON public.crushes;

-- Create balanced policies that allow matching while protecting sensitive data
-- Users can see basic profile info of others (excluding exact coordinates)
...
-- Users can view their own crushes
CREATE POLICY "Users can view their own crushes" 
ON public.crushes 
FOR SELECT 
USING (auth.uid() = sender_user_id);

-- Users can send crushes
CREATE POLICY "Users can send crushes" 
ON public.crushes 
FOR INSERT 
WITH CHECK (auth.uid() = sender_user_id);

-- Create a safe matching function that doesn't expose exact coordinates
...
GRANT EXECUTE ON FUNCTION public.get_safe_matches(numeric) TO authenticated;